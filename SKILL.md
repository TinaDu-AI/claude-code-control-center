...
- **上下文水位**：每个最近活跃的会话占了多少上下文窗口。**窗口大小按客户端自动识别**——某个客户端历史上越过过 200K，就判定它跑在 1M 窗口，它的所有会话都按 1M 算（否则 200K）。<60% 绿、60–85% 橙、≥85% 红。每条后面带那个会话的开场白，一眼知道是哪个对话。会话**正常关闭**（`/exit`、`/clear`、退出）会被 `SessionEnd` 钩子即时撤下；其它情况（如直接删除对话——应用并不会删转录文件）按「闲置超过 `SESS_WINDOW` 就撤下」兜底。手动 `/compact` 后水位即时降到开销地板，不再卡在旧值。
