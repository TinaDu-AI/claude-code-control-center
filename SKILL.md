...
- **上下文水位**：每个最近活跃的会话占了多少上下文窗口。**窗口大小按客户端自动识别**——某个客户端历史上越过过 200K，就判定它跑在 1M 窗口，它的所有会话都按 1M 算（否则 200K）。<60% 绿、60–85% 橙、≥85% 红。每条后面带那个会话的开场白，一眼知道是哪个对话。会话**关闭（`/exit`、`/clear`）或被删除**都会被 `SessionEnd` 钩子即时撤下（限装好钩子之后开的会话）；只有"开着没关、长时间不动"才按「闲置超过 `SESS_WINDOW`」兜底。手动 `/compact` 后水位即时降到开销地板，不再卡在旧值。
...
- **会话闲置多久才撤下**：`cc-stats.py` 的 `SESS_WINDOW`（默认 24h）。这是**兜底**：`/exit`、`/clear`、以及**直接删除对话**都会触发 `SessionEnd`、被即时撤下（仅限装好钩子之后才开的会话）；只有"开着没关、长时间放着不管"才靠这个 24h 闲置超时。想让"放着不管"的更快消失就调小。
